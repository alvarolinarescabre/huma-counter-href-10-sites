...
// Test HTTP client with a local mock server
func TestHTTPClient(t *testing.T) {
	originalClient := httpClient
	defer func() { httpClient = originalClient }()

	// Create a custom client with lower timeout for testing
	...
	start := time.Now()
	_, err := getURL("http://example.com:81") // Non-standard port to force timeout
